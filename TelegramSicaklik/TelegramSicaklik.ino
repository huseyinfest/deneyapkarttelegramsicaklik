...
void loop() {
  ...
  if (a == 3) {  // Saniye 300(5 dk) olduğunda telegrama veri gönderir projenize göre değiştirebilirsiniz.
    telegram.send(ID, "Sıcaklık: " + String(sicaklik) + " C°");
    a = 0;
  }
  if (sicaklik >= 35) {
    ...
    if (b == 3) { // sıcaklık 35 derecenin üstüne çıktığında 60 saniyede bir veri gönderir.
  ...
  if (sicaklik <= 25) {
    ...
    if (b == 3) { // sıcaklık 25 derecenin altına düştüğünde 60 saniyede bir veri gönderir.
